	...
	<div v-if="offerData">
		<div class="d-flex flex-column thicc-border bg-white w-100 p-2">
			<div class="d-flex col-12 justify-content-between">
				<h2 class="d-flex align-items-end mb-0">{{ offerData.Title }}</h2>
				<div class="fs-6 d-flex align-items-center">
					{{ offerData.Author.UserName }}
				</div>
			</div>
			...
			<div class="d-flex col-12">
				Car:
				{{
					`${offerData.Car.Manufacturer} ${offerData.Car.Model} ${
						offerData.Car.Type
					} ${new Date(offerData.Car.Year).getUTCFullYear()}`
				}}
			</div>
			<div class="d-flex col-12">City: {{ offerData.City.Name }}</div>
			<div v-if="offerData.Type === 'car'" class="d-flex col-12">
				Fuel: {{ offerData.Fuel }}
			</div>
			<div class="d-flex col-12">
				Condition: <ConditionChip :text="offerData.Condition" />
			</div>
			<div v-if="offerData.Type === 'car'" class="d-flex col-12">
			...
			<div v-if="offerData.Type === 'part'" class="d-flex col-12">
				Part number: {{ offerData.PartNumber }}
			</div>
			<div
				class="d-flex flex-column h-100 col-12 justify-content-between"
				style="overflow-y: auto">
				<div v-for="content in offerData.Contents" class="mb-2">
		...
		<div
			...
			<div class="d-flex justify-content-between align-items-center">
				...
				<v-btn
					:class="`text-dark ${
						offerData.UpVoted ? 'bg-secondary-subtle' : 'bg-white'
					}`"
					size="30"
					@click="handleInteraction(!offerData.UpVoted, false)"
				...
				<v-btn
					:class="`text-dark me-2 ${
						offerData.DownVoted ? 'bg-secondary-subtle' : 'bg-white'
					}`"
					size="30"
					@click="handleInteraction(false, !offerData.DownVoted)"
					:disabled="disableInteraction">
					<font-awesome-icon
						:icon="['fas', 'arrow-down']"
						style="font-size: 1em"
		...
		<div class="d-flex flex-column thicc-border bg-white w-100">
			<div class="d-flex col-12 justify-content-between">
				<div
					...
					<CommentComponent
						v-if="offerData.Comments"
						v-for="comment in offerData.Comments"
						:key="comment.CommentId"
						:data="comment"
						:addCommentCallback="handleAddSubComment" />

					<div
						v-if="!offerData.Comments || offerData.Comments.length === 0"
						class="ps-2">
...
<script setup lang="ts">
	...
	import { IComment, IOffer } from '../Intefaces'
	import { onMounted, ref } from 'vue'
	import { AddComment, GetOffer, InteractWithOffer } from '../setup/Endpoints'
	...
	const offerId = Array.isArray(route.params.id)
		? route.params.id[0]
		: route.params.id

	const offerData = ref<IOffer | null>(null)
	const offerNotFound = ref<boolean>(false)
		...
		await AddComment(commentData)
			.then((commentResponse) => {
				if (commentResponse) {
					...
					const newComments = [commentResponse, ...offerData.value.Comments]
	...
	const handleAddSubComment = (comment: IComment) => {
		offerData.value = {
			...offerData.value,
			Comments: addSubCommentRecursive(offerData.value.Comments, comment),
		}
	}
	...
	const handleInteraction = async (up: boolean, down: boolean) => {
		...
		await InteractWithOffer(interactData)
			.then(() => {
				snackBarStore.addMessage({
					text: `Successfully ${up ? 'upvoted' : 'downvoted'} offer!`,
					color: 'success',
					timeout: 1000,
				})

				const pointsToAdd =
					offerData.value.UpVoted && down
						? -2
						: offerData.value.DownVoted && up
						? 2
						: !offerData.value.UpVoted && up
						? 1
						: !offerData.value.DownVoted && down
						? -1
						: offerData.value.DownVoted && !down
						? 1
						: offerData.value.UpVoted && !up
				...
				offerData.value = {
					...
					UpVoted: up,
					DownVoted: down,
					Points: offerData.value.Points + pointsToAdd,
				}
			})
			.catch((error) => {
				console.log(error)
